...
for project in "${!repos[@]}"; do
  ...
  if [[ "$project" == "cache2k-nullaway" ]]; then
    pre_branch="pre_nullgtn_without_s_nullaway"
    post_branch="post_nullgtn_without_s_nullaway"
  elif [[ "$project" == "cache2k-CFNullness" ]]; then
    pre_branch="pre_nullgtn_without_s_CFNullness"
    post_branch="post_nullgtn_without_s_CFNullness"
  else
    pre_branch="pre_nullgtn_without_s"
    post_branch="post_nullgtn_without_s"
  fi

  clone_version "$project" "pre_nullgtn" "$pre_branch" "$url"
  clone_version "$project" "post_nullgtn" "$post_branch" "$url"
done
...
declare -A lib_copies=(
  ["picasso_pre_nullgtn"]="pre_wpi_without_s_picasso"
  ["picasso_post_nullgtn"]="post_wpi_without_s_picasso"
  ["butterknife_pre_nullgtn"]="pre_wpi_without_s_butterknife"
  ["butterknife_post_nullgtn"]="post_wpi_without_s_butterknife"
)
...
filename=$(basename "$result_file")  # e.g., table-wrapper-api_post_nullgtn-before-wpi.txt
...
for project in "${!repos[@]}"; do
  for version in pre_nullgtn post_nullgtn; do
...
echo "📤 Copying preCheckAfter.out files into results folder"

declare -a projects=("cache2k-nullaway" "picasso" "table-wrapper-api" "table-wrapper-csv-impl" "mealPlanner" "nameless")
cf_type="CFNullness"
tag="nullgtn"

for project in "${projects[@]}"; do
  for version in pre post; do
    src_file="$DATASET_DIR/${project}_${version}_${tag}/preCheckAfter.out"
    dest_file="$RESULTS_DIR/${project}_${version}_${cf_type}_${tag}.txt"

    if [ -f "$src_file" ]; then
      echo "✅ Found $src_file, copying to $dest_file"
      cp "$src_file" "$dest_file"
    else
      echo "⚠️  Missing $src_file for $project $version, skipping"
    fi
  done
done

# Handle special case: jib_post_nullgtn only
jib_src="$DATASET_DIR/jib_post_nullgtn/preCheckAfter.out"
jib_dest="$RESULTS_DIR/jib_post_CFNullness_nullgtn.txt"

if [ -f "$jib_src" ]; then
  echo "✅ Found $jib_src, copying to $jib_dest"
  cp "$jib_src" "$jib_dest"
else
  echo "⚠️  Missing $jib_src, skipping"
fi
