...
public class CacheReader {
    ...
    private static FileTime getLastModifiedTime(Path path) throws IOException {
        ...
        if (Files.isReadable(path)) {
            try (Stream<Path> fileStream = Files.walk(path)) {
                Optional<FileTime> maxLastModifiedTime = fileStream.map(subFilePath -> {
                    try {
    ...
    public CachedLayer getUpToDateLayerBySourceFiles(List<Path> sourceFiles) throws IOException, CacheMetadataCorruptedException {
        ...
        ImageLayers<CachedLayerWithMetadata> cachedLayersWithSourceFiles = cache.getMetadata().filterLayers().bySourceFiles(sourceFiles).filter();
        ...
        for (CachedLayerWithMetadata cachedLayer : cachedLayersWithSourceFiles) {
            ...
            if (sourceFilesLastModifiedTime.compareTo(cachedLayer.getMetadata().getLastModifiedTime()) <= 0) {
