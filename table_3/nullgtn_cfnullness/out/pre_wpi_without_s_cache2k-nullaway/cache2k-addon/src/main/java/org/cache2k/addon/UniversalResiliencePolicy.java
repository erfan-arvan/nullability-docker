...
public class UniversalResiliencePolicy<K, V> implements ResiliencePolicy<K, V> {
    ...
    public static final <K, V> UniversalResilienceSupplier<K, V> supplier() {
        return SUPPLIER;
    }
     ...
     */
    @Override
    public long suppressExceptionUntil(@Nullable() K key, LoadExceptionInfo<K> loadExceptionInfo, @Nullable() CacheEntry<K, V> cachedEntry) {
     ...
     */
    @Override
    public long retryLoadAfter(@Nullable() K key, LoadExceptionInfo<K> loadExceptionInfo) {
    ...
    private long calculateRetryDelta(LoadExceptionInfo loadExceptionInfo) {
        long delta = (long) (retryInterval * Math.pow(multiplier, loadExceptionInfo.getRetryCount()));
