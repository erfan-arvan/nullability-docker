...
public class Cache2kCacheMetrics extends CacheMeterBinder {
    ...
    protected void bindImplementationSpecificMetrics(MeterRegistry registry) {
        if (control.isWeigherPresent()) {
            Gauge.builder("cache.currentWeight", control, mgm -> mgm.getTotalWeight()).tags(getTagsWithCacheName()).description("The sum of weights of all cached entries.").register(registry);
        }
        if (control.isLoaderPresent()) {
            TimeGauge.builder("cache.load.duration", control, TimeUnit.MILLISECONDS, mgm -> (mgm.sampleStatistics().getMillisPerLoad())).tags(getTagsWithCacheName()).description("The time the cache has spent loading new values").register(registry);
            FunctionCounter.builder("cache.load", control, mgm -> mgm.sampleStatistics().getLoadCount()).tags(getTagsWithCacheName()).tags("result", "success").description("The number of times cache lookup methods have successfully loaded a new value").register(registry);
            FunctionCounter.builder("cache.load", control, mgm -> mgm.sampleStatistics().getLoadExceptionCount()).tags(getTagsWithCacheName()).tags("result", "failure").description("The number of times cache lookup methods threw an exception while loading a new value").register(registry);
        }
    }
    ...
    private static Iterable<Tag> extendedTags(Cache<?, ?> cache) {
        ...
        return Tags.of("cache2kCacheManager", internalCache.getCacheManager().getName(), "keyType=", internalCache.getKeyType().toString().substring(CacheType.DESCRIPTOR_TO_STRING_PREFIX.length()), "valueType=", internalCache.getKeyType().toString().substring(CacheType.DESCRIPTOR_TO_STRING_PREFIX.length()));
    }
}
