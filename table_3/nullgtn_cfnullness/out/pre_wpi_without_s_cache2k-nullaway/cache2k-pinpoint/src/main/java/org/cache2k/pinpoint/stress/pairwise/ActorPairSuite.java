...
public class ActorPairSuite {
    ...
    public void run() {
        ...
        finishTime = runMillis == Long.MAX_VALUE ? Long.MAX_VALUE : System.currentTimeMillis() + runMillis;
    ...
    private void checkForExceptions() {
        if (!exceptions.isEmpty()) {
            throw new ActorPairSuiteError(exceptions.size(), runCount.longValue(), exceptions.get(0));
        }
    }
    ...
    public static class ActorPairSuiteError extends AssertionError {
        ...
        public ActorPairSuiteError(int numExceptions, long runCount, Throwable firstException) {
            super(numExceptions + " errors of " + runCount + ", first exception: " + firstException, firstException);
        }
    }
}
