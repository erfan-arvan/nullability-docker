...
public class Pairwise<A, B> {
    ...
    public <NewB> Pairwise<A, NewB> b(@Nullable() Supplier<NewB> supplier) {
        Pairwise<A, NewB> me = (Pairwise<A, NewB>) this;
        return me;
    }

    public Pairwise<Void, B> a(@Nullable() Runnable action) {
        Pairwise<Void, B> me = (Pairwise<Void, B>) this;
        return me;
    }

    public Pairwise<A, Void> b(@Nullable() Runnable action) {
        Pairwise<A, Void> me = (Pairwise<A, Void>) this;
        return me;
    }

    public Pairwise<A, B> check(@Nullable() BiConsumer<A, B> consumer) {
        return this;
    }
}
...
class Test2 extends Pairwise<Object, Object> {
...

    {
        setup(() -> {
            counter1.set(0);
            map.remove(key);
        }).a(() -> map.compute(key, (key, value) -> {
            counter1.getAndIncrement();
            return value == null ? 1 : 11;
        })).b(() -> map.putIfAbsent(key, 2)).check((integer, integer2) -> {
        }).a(() -> {
        }).check((unused, b) -> {
        });
    }
