...
public interface EventHandling<K, V> {
    ...
    EventHandling DISABLED = new EventHandling() {
        ...
        @Override
        public boolean deregisterListener(@Nullable() final CacheEntryListenerConfiguration cfg) {
        ...
        @Override
        public void registerListener(@Nullable() final CacheEntryListenerConfiguration cfg) {
            throw new UnsupportedOperationException(ERROR);
        }

        @Override
        public void addInternalListenersToCache2kConfiguration(@Nullable() final Cache2kConfig cfg) {
        }
    };

    void registerListener(@Nullable() CacheEntryListenerConfiguration<K, V> cfg);

    @SuppressWarnings("UnusedReturnValue")
    boolean deregisterListener(@Nullable() CacheEntryListenerConfiguration<K, V> cfg);

    Collection<CacheEntryListenerConfiguration<K, V>> getAllListenerConfigurations();

    void addInternalListenersToCache2kConfiguration(@Nullable() Cache2kConfig<K, V> cfg);
}
