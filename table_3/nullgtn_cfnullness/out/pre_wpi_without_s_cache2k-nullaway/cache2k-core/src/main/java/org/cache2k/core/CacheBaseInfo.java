...
class CacheBaseInfo implements InternalCacheInfo {
   ...
   */
    ...
    CacheBaseInfo(HeapCache heapCache, InternalCache userCache, long now) {
        ...
        missCnt = metrics.getReadThroughCount() + metrics.getExplicitLoadCount() + metrics.getPeekHitNotFreshCount() + metrics.getPeekMissCount();
        ...
        totalLoadCnt = metrics.getReadThroughCount() + metrics.getExplicitLoadCount() + metrics.getRefreshCount();
    }
    ...
    public long getGetCount() {
        return hitCnt + metrics.getPeekMissCount() + metrics.getReadThroughCount() - metrics.getHeapHitButNoReadCount();
    }
    ...
    public long getRefreshCount() {
        return metrics.getRefreshCount();
    }

    @Override
    public long getInternalExceptionCount() {
        return internalExceptionCnt;
    }

    @Override
    public long getRefreshRejectedCount() {
        return metrics.getRefreshRejectedCount();
    }

    @Override
    public long getSuppressedExceptionCount() {
        return metrics.getSuppressedExceptionCount();
    }
    ...
    public long getRefreshedHitCount() {
        return metrics.getRefreshedHitCount();
    }

    @Override
    public long getExpiredCount() {
        return expiredRemoveCnt + metrics.getExpiredKeptCount();
    }
    ...
    public long getKeyMutationCount() {
        return keyMutationCnt;
    }

    @Override
    public long getTimerEventCount() {
        return metrics.getTimerEventCount();
    }
    ...
    public String toString() {
        ...
        sb.append("name=").append(BaseCache.nameQualifier(cache)).append(", ").append("size=").append(getSize()).append(", ");
    ...
    static int hashQuality(int noCollisionPercent, int longestSlot) {
        ...
        int correctionForOversizeSlot = (int) ((1 - Math.exp(EXPONENT_CONSTANT * Math.max(0, longestSlot - SLOT_SIZE_MINIMUM))) * 100);
