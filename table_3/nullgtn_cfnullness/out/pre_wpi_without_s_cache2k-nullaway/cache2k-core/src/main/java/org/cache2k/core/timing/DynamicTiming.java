...
class DynamicTiming<K, V> extends StaticTiming<K, V> {
    ...
    long calcNextRefreshTime(K key, V newObject, long now, CacheEntry<K, V> entry) {
        return calcNextRefreshTime(key, newObject, now, entry, expiryPolicy, expiryMillis, sharpExpiry);
    }

    public long calculateNextRefreshTime(Entry<K, V> entry, V newValue, long loadTime) {
        if (entry.isDataAvailable() || entry.isExpiredState() || entry.getNextRefreshTime() == Entry.EXPIRED_REFRESH_PENDING) {
            return calcNextRefreshTime(entry.getKey(), newValue, loadTime, entry.getInspectionEntry());
        } else {
