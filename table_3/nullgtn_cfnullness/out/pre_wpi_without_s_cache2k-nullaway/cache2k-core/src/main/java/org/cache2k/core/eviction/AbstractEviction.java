 ...
 * @author Jens Wilke
 */
@SuppressWarnings({ "WeakerAccess", "SynchronizationOnLocalVariableOrMethodParameter", "unchecked", "rawtypes" })
public abstract class AbstractEviction implements Eviction, EvictionMetrics {
    ...
    public AbstractEviction(HeapCacheForEviction heapCache, HeapCacheListener listener, long maxSize, Weigher weigher, long maxWeight, boolean noChunking) {
    ...
    private static int calculateChunkSize(boolean noChunking, long maxSize) {
        ...
        return Math.min(MAXIMAL_CHUNK_SIZE, MINIMAL_CHUNK_SIZE + Runtime.getRuntime().availableProcessors() - 1);
    }
    ...
    @Override
    public void evictEventuallyBeforeInsertOnSegment(@Nullable() int hashCodeHint) {
    ...
    public String getExtraStatistics() {
        String s = "impl=" + this.getClass().getSimpleName() + ", chunkSize=" + chunkSize;
        if (isWeigherPresent()) {
            s += ", maxWeight=" + maxWeight + ", totalWeight=" + totalWeight;
        } else {
            s += ", maxSize=" + maxSize;
        }
        s += ", size=" + getSize();
     ...
     */
    protected abstract void insertIntoReplacementList(@Nullable() Entry e);
     ...
     */
    protected abstract void removeFromReplacementList(@Nullable() Entry e);
