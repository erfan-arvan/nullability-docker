...
public class RejectNullValueTest {
    ...
    @ClassRule
    public final static CacheRule<Integer, Integer> staticTarget = new IntCacheRule().config(new CacheRule.Specialization<Integer, Integer>() {
    ...
    static void configureRejectNull(final Cache2kBuilder<Integer, Integer> b) {
        ...
        b.expiryPolicy(new ExpiryPolicy<Integer, Integer>() {
            ...
            @Override
            public long calculateExpiryTime(final Integer key, final Integer value, @Nullable() final long loadTime, @Nullable() final CacheEntry<Integer, Integer> currentEntry) {
                if (key >= 8) {
