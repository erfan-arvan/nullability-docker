...
public class TimerTest {
    ...
    public void misc() {
        long startTime = 10;
        Timer st = new DefaultTimer(new SimulatedClock(startTime), 1);
    ...
    public void config() {
        long lag = hashCode();
        assertEquals("builder and config bean working", lag, Cache2kBuilder.forUnknownTypes().timerLag(lag, TimeUnit.MILLISECONDS).config().getTimerLag().toMillis());
        assertEquals("eternal / overflow", Long.MAX_VALUE, Cache2kBuilder.forUnknownTypes().timerLag(Long.MAX_VALUE, TimeUnit.SECONDS).config().getTimerLag().toMillis());
        assertSame("eternal / overflow", Cache2kConfig.EXPIRY_ETERNAL, Cache2kBuilder.forUnknownTypes().timerLag(Long.MAX_VALUE, TimeUnit.SECONDS).config().getTimerLag());
    }
    ...
    class MyTimerTask extends TimerTask implements Comparable<MyTimerTask> {
        ...
        public String toString() {
            return "MyTimerTask{" + "id=" + id + ", scheduleTime=" + scheduleTime + ", executed=" + executed + ", super=" + super.toString() + '}';
        }
    ...
    static class MyClock implements InternalClock, Scheduler {
        ...
        @Override
        public void sleep(@Nullable() long millis) {
            assert false;
        }
