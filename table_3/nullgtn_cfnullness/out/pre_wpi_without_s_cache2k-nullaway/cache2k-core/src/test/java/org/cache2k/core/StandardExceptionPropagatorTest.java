...
public class StandardExceptionPropagatorTest {
    ...
    static final long SOME_TIME = LocalDateTime.parse(TIME_STRING).atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();

    @Test
    public void propagate_eternal() {
        System.currentTimeMillis();
        RuntimeException t = STANDARD_PROPAGATOR.propagateException(toInfo(new RuntimeException("serious thing"), Long.MAX_VALUE));
        assertTrue(t.toString().contains("expiry=ETERNAL"));
    }

    @Test
    public void propagate_sometime() {
        RuntimeException t = STANDARD_PROPAGATOR.propagateException(toInfo(new RuntimeException("serious thing"), SOME_TIME));
        assertThat(t.toString()).contains(("expiry=2016-05-25T09:30:12.123"));
    }

    @Test
    public void propagate_notime() {
        RuntimeException t = STANDARD_PROPAGATOR.propagateException(toInfo(new RuntimeException("serious thing"), 0));
