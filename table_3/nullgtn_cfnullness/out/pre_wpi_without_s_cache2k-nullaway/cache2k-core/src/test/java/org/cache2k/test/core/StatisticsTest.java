...
public class StatisticsTest extends TestingBase {
    ...
    public void testGetFetch() {
        final Cache<Integer, Integer> c = freshCache(new IdentIntSource(), 100);
        c.get(3);
        assertEquals(1, getInfo().getMissCount());
    }
    ...
    public void testGetFetchAlwaysOneGet() {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).loader(g).expireAfterWrite(0, TimeUnit.SECONDS).build();
    ...
    public void testGetFetchAndRefresh(boolean keepData) throws Exception {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).keepDataAfterExpired(keepData).loader(g).refreshAhead(true).expireAfterWrite(expiryMillis, TimeUnit.MILLISECONDS).build();
        assertEquals("no miss yet", 0, g.getLoaderCalledCount());
    ...
    public void testReload() throws Exception {
        IntCountingCacheSource g = new IntCountingCacheSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).entryCapacity(1).loader(g).sharpExpiry(true).expireAfterWrite(2, TimeUnit.MILLISECONDS).build();
        // new entry and load miss
        c.get(1802);
        c.get(1803);
        c.get(1804);
        // new entry and load hit, (if within < 2 ms)
        c.get(1802);
        // direct hit
        c.get(1802);
        sleep(2);
    ...
    void testUsageCounter(int[] accessPattern, int size) throws Exception {
        IntCountingCacheSource g = new IntCountingCacheSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).entryCapacity(size).loader(g).eternal(true).build();
    ...
    int[] accessPattern1 = new int[] { 1, 2, 3, 1, 1, 1, 2, 3, 4, 5, 5, 5, 2, 3, 4, 5, 6, 7, 8, 9 };
    ...
    int[] accessPattern2 = new int[] { 1, 2, 3, 1, 1, 4, 5, 1, 2, 3, 4, 4, 4 };
    ...
    public void toStringWithEmptyCache() {
        IntCountingCacheSource g = new IntCountingCacheSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).loader(g).eternal(true).build();
    ...
    public void disableStatistics() {
        IntCountingCacheSource g = new IntCountingCacheSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).loader(g).disableStatistics(true).eternal(true).build();
        c.get(1);
        c.remove(1);
        assertEquals(0, getInfo().getLoadCount());
        c.toString();
