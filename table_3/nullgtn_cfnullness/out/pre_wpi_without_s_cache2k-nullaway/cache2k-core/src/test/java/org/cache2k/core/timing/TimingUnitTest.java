...
public class TimingUnitTest {
    ...
    private <K, V> Timing<K, V> create(final InternalClock clock, final Cache2kConfig<K, V> cfg) {
    ...
    public void eternalSpecified() {
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().eternal(true).config());
        assertEquals(TimeAgnosticTiming.ETERNAL_IMMEDIATE.getClass(), h.getClass());
    }

    @Test
    public void eternalNotSpecified() {
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().config());
        assertEquals(TimeAgnosticTiming.ETERNAL_IMMEDIATE.getClass(), h.getClass());
    }

    @Test
    public void expireAfterWrite_overflow() {
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().expireAfterWrite(Long.MAX_VALUE - 47, TimeUnit.SECONDS).config());
        assertEquals(TimeAgnosticTiming.ETERNAL_IMMEDIATE.getClass(), h.getClass());
    }
    ...
    public void almostEternal_noOverflow() {
        ...
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().expireAfterWrite(bigValue, TimeUnit.MILLISECONDS).config());
    ...
    public void almostEternal_expiryPolicy_noOverflow() {
        long bigValue = Long.MAX_VALUE - 47;
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().expiryPolicy(new ExpiryPolicy() {

            @Override
            public long calculateExpiryTime(@Nullable() Object key, @Nullable() Object value, @Nullable() long loadTime, @Nullable() CacheEntry currentEntry) {
                return ETERNAL;
            }
        }).expireAfterWrite(bigValue, TimeUnit.MILLISECONDS).config());
        long t = h.calculateNextRefreshTime(ENTRY, null, 0);
        assertEquals(bigValue, t);
        t = h.calculateNextRefreshTime(ENTRY, null, 48);
        assertEquals(Long.MAX_VALUE, t);
    ...
    public void policy_sharp() {
        Timing h = create(CLOCK, Cache2kBuilder.forUnknownTypes().expiryPolicy(new ExpiryPolicy() {
            ...
            @Override
            public long calculateExpiryTime(@Nullable() Object key, @Nullable() Object value, long loadTime, @Nullable() CacheEntry currentEntry) {
                return loadTime + 1;
            }
        }).sharpExpiry(true).config());
        Entry e = new Entry();
    ...
    public void expireAfterWrite_policy_limit() {
        ...
        }).expireAfterWrite(5, TimeUnit.MINUTES).config());
        Entry e = new Entry();
        long t = h.calculateNextRefreshTime(e, null, NOW);
    ...
    public void expireAfterWrite_policy_limit_sharp() {
        ...
        }).expireAfterWrite(duration, TimeUnit.MILLISECONDS).config());
        Entry e = new Entry();
        long t = h.calculateNextRefreshTime(e, null, NOW);
        assertNotEquals(Long.MAX_VALUE, t);
        assertEquals("max expiry, but not sharp", NOW + duration, t);
    ...
    public void expireAfterWrite_policy_limit_sharp_close() {
        ...
        assertTrue("expect gap bigger then duration", HeapCache.TUNABLE.sharpExpirySafetyGapMillis > duration);
