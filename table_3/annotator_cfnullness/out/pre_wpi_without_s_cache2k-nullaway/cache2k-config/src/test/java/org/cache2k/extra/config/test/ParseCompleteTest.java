...
public class ParseCompleteTest {
    ...
    public void parseIt() throws Exception {
        ...
        assertEquals("true", defaults.getSection("cache").getPropertyMap().get("suppressExceptions").getValue());
        ParsedConfiguration caches = topLevel.getSection("caches");
        assertNotNull(caches);
        assertEquals("5", caches.getSection("products").getPropertyMap().get("entryCapacity").getValue());
        assertNotNull("cache has eviction section", caches.getSection("products").getSection("eviction"));
        assertEquals("123", caches.getSection("products").getSection("eviction").getPropertyMap().get("aValue").getValue());
        assertEquals("123", topLevel.getStringPropertyByPath("caches.products.eviction.aValue"));
    ...
    public void parseAndExpand() throws Exception {
        ...
        assertEquals(homeDirectory, cfg.getStringPropertyByPath("caches.products.eviction.directory"));
    ...
    List<String> extractNames(Collection<ParsedConfiguration> lc) {
        List<String> l = new ArrayList<String>();
        for (ParsedConfiguration c : lc) if (c.getName() != null) {
            l.add(c.getName());
        }
        return l;
    }
