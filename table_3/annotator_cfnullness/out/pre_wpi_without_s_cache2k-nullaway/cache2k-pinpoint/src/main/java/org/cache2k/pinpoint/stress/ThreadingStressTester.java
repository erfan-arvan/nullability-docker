...
public class ThreadingStressTester {
        ...
        try {
            ...
            if (!allStopped) {
                ...
                if (timeoutAfterInterrupt) {
                    String timeoutReason = "Threads ignoring interrupt signal. " + "Timeout after " + (stopTime - startTime) + " milliseconds.";
    ...
    private void printTimeoutInfo(String reason) {
        ...
        errorOutput.println("testTimeMillis=" + testTimeMillis + ", timeoutMillis=" + oneShotTimeoutMillis + ", timeoutAfterInterruptMillis=" + timeoutAfterInterruptMillis);
    ...
    private void propagateException() {
        ...
        if (gotAllAssertionErrors()) {
            throw new AssertionError("Got " + exceptions.size() + " thread assertion error(s). First one propagated.", exceptions.get(0));
        } else {
            throw new ThreadingStressTestException("Got " + exceptions.size() + " thread exceptions(s). First one propagated.", exceptions.get(0));
        }
    }
    ...
    private void printThreadExceptions() {
        ...
        errorOutput.println(exceptions.size() + " " + "exceptions detected during test run.");
        for (Throwable t : exceptions) {
