...
class LifecycleListener implements CacheCreatedListener, CacheClosedListener {
    ...
    @Override
    @Nullable()
    public <K, V> CompletableFuture<Void> onCacheCreated(Cache<K, V> cache, @Nullable() CacheBuildContext<K, V> ctx) {
    ...
    private static String createCacheControlName(CacheManager cm, Cache<?, ?> c) {
        return "org.cache2k" + ":" + "type=Cache" + ",manager=" + sanitizeNameAsJmxValue(cm.getName()) + ",name=" + sanitizeNameAsJmxValue(c.getName());
    }

    private static String createCacheStatisticsName(CacheManager cm, Cache<?, ?> c) {
        return "org.cache2k" + ":" + "type=CacheStatistics" + ",manager=" + sanitizeNameAsJmxValue(cm.getName()) + ",name=" + sanitizeNameAsJmxValue(c.getName());
    }
