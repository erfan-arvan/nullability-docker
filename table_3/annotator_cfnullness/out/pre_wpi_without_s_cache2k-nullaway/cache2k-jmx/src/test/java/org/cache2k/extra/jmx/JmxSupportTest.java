...
public class JmxSupportTest {
    ...
    private static ObjectName getCacheManagerObjectName(String name) throws MalformedObjectNameException {
    ...
    public void testCacheCreated() throws Exception {
        ...
        Cache c = Cache2kBuilder.of(Object.class, Object.class).name(name).setup(JmxSupport::enable).eternal(true).build();
    ...
    public void testCacheCreated_notFound() throws Exception {
        String name = getClass().getName() + ".testCacheCreated";
        Cache c = Cache2kBuilder.of(Object.class, Object.class).name(name).eternal(true).build();
        MBeanInfo i = getCacheInfo(name);
        c.close();
    }
    ...
    public void testInitialProperties() throws Exception {
        ...
        }.name(name).eternal(true).setup(JmxSupport::enable).build();
        ...
        assertTrue("reasonable CreatedTime", ((Date) retrieve("CreatedTime")).compareTo(beforeCreation) >= 0);
    ...
    public void testDisabledStatistics() throws Exception {
        ...
        Cache c = new Cache2kBuilder<Long, List<Collection<Long>>>() {
        }.name(name).disableStatistics(true).setup(JmxSupport::enable).build();
        objectName = constructCacheObjectName(name);
        checkAttribute("KeyType", "Long");
        objectName = constructCacheStatisticsObjectName(name);
        assertThatCode(() -> retrieve("HitRate")).isInstanceOf(InstanceNotFoundException.class);
        c.clear();
    }
    ...
    public void testDisableMonitoring() throws Exception {
        ...
        Cache c = new Cache2kBuilder<Long, List<Collection<Long>>>() {
        }.name(name).setup(JmxSupport::enable).disableMonitoring(true).build();
        objectName = constructCacheObjectName(name);
        assertThatCode(() -> retrieve("KeyType")).isInstanceOf(InstanceNotFoundException.class);
        objectName = constructCacheStatisticsObjectName(name);
        assertThatCode(() -> retrieve("HitRate")).isInstanceOf(InstanceNotFoundException.class);
        c.clear();
    }
    ...
    public void testWeigherWithSegmentation() throws Exception {
        ...
        Cache c = new Cache2kBuilder<Long, List<Collection<Long>>>() {
        }.name(name).eternal(true).disableStatistics(false).setup(JmxSupport::enable).maximumWeight(123456789L).weigher(new Weigher<Long, List<Collection<Long>>>() {

            @Override
            public int weigh(@Nullable() Long key, @Nullable() List<Collection<Long>> value) {
                return 1;
            }
        }).build();
        objectName = constructCacheObjectName(name);
    ...
    public void testDisabled() throws Exception {
        String name = getClass().getName() + ".testInitialProperties";
        Cache c = new Cache2kBuilder<Long, List<Collection<Long>>>() {
        }.name(name).disableStatistics(true).eternal(true).build();
        objectName = constructCacheObjectName(name);
        try {
            retrieve("Alert");
            fail("exception expected");
    ...
    private Object retrieve(String name) throws MBeanException, AttributeNotFoundException, InstanceNotFoundException, ReflectionException, IOException {
    ...
    static ObjectName constructCacheStatisticsObjectName(String name) throws MalformedObjectNameException {
        return new ObjectName("org.cache2k:type=CacheStatistics,manager=default," + "name=" + maybeQuote(name));
    }

    @Test(expected = InstanceNotFoundException.class)
    public void testCacheDestroyed() throws Exception {
        String name = getClass().getName() + ".testCacheDestroyed";
        Cache c = Cache2kBuilder.of(Object.class, Object.class).name(name).eternal(true).setup(JmxSupport::enable).build();
        MBeanInfo i = getCacheInfo(name);
        c.close();
        getCacheInfo(name);
    }
    ...
    public void testEnableDisable() throws Exception {
        ...
        Cache c = Cache2kBuilder.of(Object.class, Object.class).name(name).eternal(true).setup(JmxSupport::enable).setup(JmxSupport::disable).build();
        MBeanInfo i = getCacheInfo(name);
        c.close();
        getCacheInfo(name);
    }
