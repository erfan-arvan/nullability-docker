...
public class Cache2kBuilderTest {
    ...
    public void forUnknownTypes_genericTyping() {
        Cache<Object, Object> cache = Cache2kBuilder.forUnknownTypes().with(CoreConfig.class, b -> b.timerReference(new SimulatedClock())).loader(key -> key).build();
        cache.put(123, 555);
        cache.put("123", "314");
        Cache raw = cache;
        raw.close();
    ...
    public void fromConfigBean() {
        Cache<Integer, Integer> cache = new Cache2kConfig<Integer, Integer>().builder().build();
    ...
    public void typesParameters() {
        Cache<Long, String> c = Cache2kBuilder.forUnknownTypes().valueType(String.class).keyType(Long.class).eternal(true).build();
    ...
    public void noTypesCastObjectObject() {
        Cache<Object, Object> c = (Cache<Object, Object>) Cache2kBuilder.forUnknownTypes().eternal(true).build();
        c.put("hallo", 234);
        c.close();
    }
    ...
    public void noTypesCastStringInt() {
        Cache<String, Integer> c = (Cache<String, Integer>) (Cache) Cache2kBuilder.forUnknownTypes().eternal(true).build();
        c.put("hallo", 234);
        c.close();
    }
    ...
    public void collectionValueType() {
        Cache<Long, List<String>> c = new Cache2kBuilder<Long, List<String>>() {
        }.eternal(true).build();
        c.close();
    }
    ...
    public void collectionValueClass() {
        Cache<Long, List<String>> c = (Cache<Long, List<String>>) (Object) Cache2kBuilder.of(Long.class, List.class).eternal(true).build();
    ...
    public void collectionValueCacheType() {
        Cache<Long, List<String>> c = Cache2kBuilder.forUnknownTypes().keyType(Long.class).valueType(new CacheTypeCapture<List<String>>() {
        }).eternal(true).build();
        c.put(123L, new ArrayList<String>());
        c.close();
    }
    ...
    public void typesParametersWithList() {
        Cache<Long, List> c = Cache2kBuilder.forUnknownTypes().valueType(List.class).keyType(Long.class).eternal(true).build();
        c.close();
    }

    @Test
    public void noTypesAndCast() {
        Cache<Long, List<String>> c = (Cache<Long, List<String>>) (Cache) Cache2kBuilder.forUnknownTypes().eternal(true).build();
        c.close();
    }
    ...
    public void cacheNameForAnnotationDefault() {
        Cache<Long, List<String>> c = (Cache<Long, List<String>>) (Cache) Cache2kBuilder.forUnknownTypes().eternal(true).name("package.name.ClassName.methodName(package.ParameterType,package.ParameterType").build();
        c.close();
    }
    ...
    public void managerAfterOtherStuff() {
        Cache2kBuilder.forUnknownTypes().eternal(true).manager(CacheManager.getInstance()).build();
    }
    ...
    final static String ILLEGAL_CHARACTERS_IN_NAME = "{}|\\^&=\";:<>*?/" + ((char) 27) + ((char) 127) + ((char) 128) + "äßà\ufefe";
    ...
    private void cacheClosedEventFired(boolean _wiredCache) {
        ...
        _builder = _builder.addCacheClosedListener(new CacheClosedListener() {
            ...
            @Override
            @Nullable()
            public CompletableFuture<Void> onCacheClosed(@Nullable() final Cache cache) {
                _FIRED.set(true);
                return null;
            }
        });
    ...
    public void refreshAheadButNoLoader() {
        Cache c = Cache2kBuilder.forUnknownTypes().refreshAhead(true).expireAfterWrite(10, TimeUnit.SECONDS).build();
        c.close();
    }

    @Test(expected = IllegalArgumentException.class)
    public void refreshAheadButNoExpiry() {
        Cache c = Cache2kBuilder.forUnknownTypes().loader(new FunctionalCacheLoader() {

            @Override
            public Object load(@Nullable() final Object key) throws Exception {
                return null;
            }
    ...
    static class BuildCacheInClassConstructor0 {
        ...
        static Cache<?, ?> cache = Cache2kBuilder.forUnknownTypes().eternal(true).build();
    }
}
