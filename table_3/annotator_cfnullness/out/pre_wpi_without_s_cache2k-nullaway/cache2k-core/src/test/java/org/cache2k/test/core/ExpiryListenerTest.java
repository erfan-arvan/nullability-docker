...
public class ExpiryListenerTest extends TestingBase {
    ...
    public void expireBeforePut() {
        ...
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).addListener(new CacheEntryExpiredListener<Integer, Integer>() {
    ...
    public void expireAfterRefreshProbationEnded() {
        ...
        final long expiryMillis = TestingParameters.MINIMAL_TICK_MILLIS;
        Cache2kBuilder<Integer, Integer> builder = builder(Integer.class, Integer.class).loader(new AdvancedCacheLoader<Integer, Integer>() {

            @Override
            public Integer load(Integer key, @Nullable() long startTime, @Nullable() CacheEntry<Integer, Integer> currentEntry) {
                loaderCount.getAndIncrement();
                assertEquals(0, eventCount.get());
                return key;
            }
        }).expireAfterWrite(expiryMillis, TimeUnit.MILLISECONDS).refreshAhead(true).addListener(new CacheEntryExpiredListener<Integer, Integer>() {
    ...
    private void expireBeforeLoad(boolean sharpExpiry) {
        ...
        Cache2kBuilder<Integer, Integer> builder = builder(Integer.class, Integer.class).loader(new AdvancedCacheLoader<Integer, Integer>() {
            ...
            @Override
            public Integer load(@Nullable() Integer key, @Nullable() long startTime, CacheEntry<Integer, Integer> currentEntry) {
        ...
        }).keepDataAfterExpired(true).addListener(new CacheEntryExpiredListener<Integer, Integer>() {
    ...
    private void testListenerCalled(boolean sharp, boolean beyondSafetyGap) {
        final AtomicInteger callCount = new AtomicInteger();
        HeapCache.Tunable t = TunableFactory.get(HeapCache.Tunable.class);
        long expiryMillis = (beyondSafetyGap ? t.sharpExpirySafetyGapMillis : 0) + TestingParameters.MINIMAL_TICK_MILLIS;
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).addListener(new CacheEntryExpiredListener<Integer, Integer>() {

            @Override
            public void onEntryExpired(@Nullable() Cache<Integer, Integer> c, @Nullable() CacheEntry<Integer, Integer> e) {
                callCount.incrementAndGet();
            }
        }).expireAfterWrite(expiryMillis, TimeUnit.MILLISECONDS).sharpExpiry(sharp).build();
        final int anyKey = 1;
        within(expiryMillis).perform(new Runnable() {
    ...
    public void asyncExpiredListenerAfterRefreshCalled() {
        ...
        }).expireAfterWrite(expiryMillis, TimeUnit.MILLISECONDS).refreshAhead(true).loader(new CacheLoader<Integer, Integer>() {
    ...
    public void expiresDuringInsert() throws InterruptedException {
        ...
        }).expireAfterWrite(expiryMillis, TimeUnit.MILLISECONDS).loader(new CacheLoader<Integer, Integer>() {
