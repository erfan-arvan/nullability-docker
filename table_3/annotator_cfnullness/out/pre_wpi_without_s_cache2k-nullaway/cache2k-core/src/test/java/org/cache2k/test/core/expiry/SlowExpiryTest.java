...
public class SlowExpiryTest extends TestingBase {
    ...
    public void testExceptionWithRefreshAndLoader(boolean asyncLoader) {
        ...
        Cache2kBuilder<Integer, Integer> cb = builder(cacheName, Integer.class, Integer.class).refreshAhead(true).resiliencePolicy(new ExpiryTest.EnableExceptionCaching(timespan));
        if (asyncLoader) {
            cb.loader(new AsyncCacheLoader<Integer, Integer>() {

                @Override
                public void load(@Nullable() Integer key, @Nullable() Context<Integer, Integer> context, @Nullable() Callback<Integer> callback) {
        ...
        cache = c;
        within(timespan).perform(new Runnable() {
            ...
            public void run() {
                ...
                assertEquals("no refresh yet", 0, getInfo().getRefreshCount() + getInfo().getRefreshRejectedCount());
            }
        });
    ...
    public void testExceptionWithRefreshSyncLoader() {
        ...
        final int count = 4;
        final long timespan = TestingParameters.MINIMAL_TICK_MILLIS;
        final Cache<Integer, Integer> c = builder(cacheName, Integer.class, Integer.class).refreshAhead(true).resiliencePolicy(new ExpiryTest.EnableExceptionCaching(timespan)).loader(key -> {
            throw new RuntimeException("always");
        }).build();
        cache = c;
        within(timespan).perform(new Runnable() {
    ...
    public void testExceptionExpirySuppressTwiceWaitForExceptionExpiry() {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expiryPolicy((key, value1, loadTime, oldEntry) -> 0).resiliencePolicy(new ResiliencePolicy<Integer, Integer>() {

            @Override
            public long suppressExceptionUntil(@Nullable() Integer key, LoadExceptionInfo<Integer> loadExceptionInfo, @Nullable() CacheEntry<Integer, Integer> cachedEntry) {
                return loadExceptionInfo.getLoadTime() + exceptionExpiryMillis;
            }

            @Override
            public long retryLoadAfter(@Nullable() Integer key, @Nullable() LoadExceptionInfo<Integer> loadExceptionInfo) {
                return 0;
            }
        }).keepDataAfterExpired(true).loader(src).build();
        c.get(2);
        within(exceptionExpiryMillis).perform(() -> {
            // exception gets suppressed
            c.get(2);
        }).expectMaybe(() -> {
        ...
        await(TestingParameters.MAX_FINISH_WAIT_MILLIS, new Condition() {
            ...
            @Override
            public boolean check() {
                // value is fetched, again if expired
                c.get(2);
    ...
    public void testExceptionExpiryNoSuppress() {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expiryPolicy((key, value1, loadTime, oldEntry) -> 0).resiliencePolicy(new ExpiryTest.EnableExceptionCaching(TestingParameters.MINIMAL_TICK_MILLIS)).loader(src).build();
        ...
        exceptionToString = exceptionToString.replace(".#,", ".###,");
        exceptionToString = exceptionToString.replace("##:##,", "##:##.###,");
        assertEquals("org.cache#k.io.CacheLoaderException: " + "expiry=####-##-##T##:##:##.###, cause: " + "java.lang.RuntimeException: every # times", exceptionToString);
        exceptionCount = 0;
        try {
            // value is fetched
            c.get(2);
            // value is fetched again (expiry=0), but exception happens, no suppress
            c.get(2);
        } catch (CacheLoaderException e) {
            exceptionCount++;
        }
    ...
    public void testSuppressExceptionImmediateExpiry() {
        BasicCacheTest.OccasionalExceptionSource src = new BasicCacheTest.OccasionalExceptionSource();
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expiryPolicy((key, value1, loadTime, oldEntry) -> 0).resiliencePolicy(new ResiliencePolicy<Integer, Integer>() {
    ...
    public void testSuppressExceptionShortExpiry() {
        BasicCacheTest.OccasionalExceptionSource src = new BasicCacheTest.OccasionalExceptionSource();
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(TestingParameters.MINIMAL_TICK_MILLIS, TimeUnit.MILLISECONDS).resiliencePolicy(new ResiliencePolicy<Integer, Integer>() {
    ...
    public void testShortExpiryTimeDelayLoad() {
        ...
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(1, TimeUnit.MILLISECONDS).keepDataAfterExpired(keepData).loader(new CacheLoader<Integer, Integer>() {
    ...
    public void testNoSuppressExceptionShortExpiry() {
        BasicCacheTest.OccasionalExceptionSource src = new BasicCacheTest.OccasionalExceptionSource();
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(TestingParameters.MINIMAL_TICK_MILLIS, TimeUnit.MILLISECONDS).keepDataAfterExpired(false).loader(src).build();
        c.get(2);
        await(new Condition() {
    ...
    public void testSuppressExceptionLongExpiryAndReload() {
        BasicCacheTest.OccasionalExceptionSource src = new BasicCacheTest.OccasionalExceptionSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(TestingParameters.MAX_FINISH_WAIT_MILLIS, TimeUnit.MINUTES).resiliencePolicy(new ResiliencePolicy<Integer, Integer>() {
    ...
    public void testNeverSuppressWithRetryInterval0() {
        BasicCacheTest.OccasionalExceptionSource src = new BasicCacheTest.OccasionalExceptionSource();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(TestingParameters.MAX_FINISH_WAIT_MILLIS, TimeUnit.MINUTES).loader(src).build();
        neverSuppressBody(c);
    }
    ...
    private void neverSuppressBody(Cache<Integer, Integer> c) {
        final int key = 2;
        c.get(key);
        assertThatCode(() -> c.reloadAll(asList(key)).get()).getCause().isInstanceOf(CacheLoaderException.class);
        ...
        assertThatCode(() -> c.get(key)).doesNotThrowAnyException();
        assertEquals(0, getInfo().getSuppressedExceptionCount());
    ...
    public void testExpireNoKeepSharpExpiryBeyondSafetyGap() {
        ...
        Cache<Integer, Integer> c = cache = builder(Integer.class, Integer.class).loader(new IntCountingCacheSource()).expireAfterWrite(t.sharpExpirySafetyGapMillis + 3, TimeUnit.MILLISECONDS).keepDataAfterExpired(false).sharpExpiry(true).build();
    ...
    public void testExpireNoKeepAsserts() {
        final Cache<Integer, Integer> c = cache = builder(Integer.class, Integer.class).loader(new IntCountingCacheSource()).expireAfterWrite(TestingParameters.MINIMAL_TICK_MILLIS, TimeUnit.MILLISECONDS).keepDataAfterExpired(false).build();
    ...
    public void testExpireNoKeep(long millis) {
        final Cache<Integer, Integer> c = cache = builder(Integer.class, Integer.class).loader(new IntCountingCacheSource()).expireAfterWrite(millis, TimeUnit.MILLISECONDS).keepDataAfterExpired(false).build();
        within(millis).perform(new Runnable() {
    ...
    public void expireLoaded_sharp_noKeep() {
        Cache<Integer, Integer> c = cache = builder(Integer.class, Integer.class).loader(new IntCountingCacheSource()).expireAfterWrite(TestingParameters.MINIMAL_TICK_MILLIS, TimeUnit.MILLISECONDS).keepDataAfterExpired(false).sharpExpiry(true).build();
        c.getAll(toIterable(1, 2, 3));
        await(new Condition() {
    ...
    public void refreshAndSharp_get(boolean keepData, final long tickTime) {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).sharpExpiry(true).keepDataAfterExpired(keepData).expiryPolicy(new ExpiryPolicy<Integer, Integer>() {
        ...
        assertThat("minimum loads triggered", getInfo().getLoadCount(), greaterThanOrEqualTo(loadsTriggeredByGet));
        assertThat("maximum loads triggered", getInfo().getLoadCount(), lessThanOrEqualTo(loadsTriggeredByGet + additionalLoadsBecauseOfRefresh));
        if (tickTime > 0) {
    ...
    public void refresh_sharp_regularExpireAfterWriter_lagging() throws Exception {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).sharpExpiry(true).expireAfterWrite(expiry, TimeUnit.MILLISECONDS).loader(new CacheLoader<Integer, Integer>() {
        ...
        within(expiry).perform(new Runnable() {
    ...
    public void refresh_sharp_noKeep(final long expiry) {
        ...
        final int key = 1;
        final CountingLoader loader = new CountingLoader();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).sharpExpiry(true).eternal(true).keepDataAfterExpired(false).expiryPolicy(new ExpiryPolicy<Integer, Integer>() {

            @Override
            public long calculateExpiryTime(@Nullable() Integer key, @Nullable() Integer value, long loadTime, CacheEntry<Integer, Integer> currentEntry) {
        ...
        within(maxFinishWaitMillis + expiry).perform(new Runnable() {
    ...
    public void refresh_sharp_keep() {
        final int key = 1;
        final CountingLoader loader = new CountingLoader();
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).sharpExpiry(true).eternal(true).keepDataAfterExpired(true).expiryPolicy(new ExpiryPolicy<Integer, Integer>() {
        ...
        within(TestingParameters.MAX_FINISH_WAIT_MILLIS).perform(new Runnable() {
    ...
    public void refresh_timerStoppedWithRemove() throws InterruptedException {
        ...
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).expireAfterWrite(expiry, TimeUnit.MILLISECONDS).loader(loader).build();
        long t0 = millis();
        c.get(key);
        sleep(expiry * 3 / 2);
        c.remove(key);
        if (millis() - t0 < expiry * 2) {
            ...
            assertThat("0 timer events if we are to fast, " + "max. 1 timer event because entry was removed before the refresh could happen", getInfo().getTimerEventCount(), isOneOf(0L, 1L));
        }
    }
    ...
    public void refresh_secondTimerEvent_allIsCleared() throws InterruptedException {
        ...
        CountingLoader loader = new CountingLoader();
        final long expiry = 123;
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).refreshAhead(true).expireAfterWrite(expiry, TimeUnit.MILLISECONDS).loader(loader).keepDataAfterExpired(false).build();
        c.get(key);
        await(new Condition() {
    ...
    public void loadAndExpireRaceNoGone() {
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).keepDataAfterExpired(false).expireAfterWrite(TestingParameters.MINIMAL_TICK_MILLIS, TimeUnit.MILLISECONDS).loader(new CacheLoader<Integer, Integer>() {
    ...
    public void manualExpire_nowIsh() {
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(5, TimeUnit.MINUTES).build();
        c.put(1, 2);
    ...
    private void expireAt_x_doesRefresh(long x) {
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(TestingParameters.MAX_FINISH_WAIT_MILLIS, TimeUnit.MILLISECONDS).refreshAhead(true).loader(new CacheLoader<Integer, Integer>() {
    ...
    public void timerLag_raisedLag() {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).expireAfterWrite(1, TimeUnit.MILLISECONDS).timerLag(lagMillis, TimeUnit.MILLISECONDS).build();
    ...
    public void neutralWhenModified() throws Exception {
        ...
        final Cache<Integer, Integer> c = builder(Integer.class, Integer.class).sharpExpiry(true).expiryPolicy(new ExpiryPolicy<Integer, Integer>() {
    ...
    public void neutralWhenCreatedYieldsException() throws Exception {
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).sharpExpiry(true).expiryPolicy(new ExpiryPolicy<Integer, Integer>() {
    ...
    public void expiryPolicy_dontCache_load_exception() {
        Cache<Integer, Integer> c = builder(Integer.class, Integer.class).sharpExpiry(true).expiryPolicy((key, value, loadTime, oldEntry) -> loadTime + 100).loader(key -> {
