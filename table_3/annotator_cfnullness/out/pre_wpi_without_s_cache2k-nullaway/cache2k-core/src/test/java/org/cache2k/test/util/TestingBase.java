...
public class TestingBase {
     ...
     */
    public static final Executor SHARED_EXECUTOR = new ThreadPoolExecutor(4, 8 * Runtime.getRuntime().availableProcessors(), 21, TimeUnit.SECONDS, new SynchronousQueue<Runnable>(), HeapCache.TUNABLE.threadFactoryProvider.newThreadFactory("test-loader-pool"), new ThreadPoolExecutor.AbortPolicy());
    ...
    @Rule
    public TestRule checkAndCleanup = RuleChain.outerRule(new ExternalResource() {
        ...
        @Override
        public Statement apply(final Statement base, @Nullable() Description description) {
    ...
    protected <K, T> Cache2kBuilder<K, T> builder(String cacheName, Class<K> k, Class<T> t) {
        ...
        Cache2kBuilder<K, T> b = Cache2kBuilder.of(k, t).with(CoreConfig.class, b2 -> b2.timerReference(getClock())).name(cacheName).entryCapacity(DEFAULT_MAX_SIZE).timerLag(TestingParameters.MINIMAL_TICK_MILLIS / 2, TimeUnit.MILLISECONDS).loaderExecutor(loaderExecutor).executor(asyncExecutor);
     ...
     * @param b
     */
    protected void applyAdditionalOptions(@Nullable() Cache2kBuilder b) {
    }
    ...
    @Nullable()
    protected <K, T> Cache<K, T> freshCache(Class<K> keyClass, Class<T> dataClass, CacheLoader g, long maxElements, int expiry) {
        Cache2kBuilder<K, T> b = builder(keyClass, dataClass).setup(x -> {
            if (g != null)
                x.loader(g);
    ...
    protected Cache<Integer, Integer> freshCache(CacheLoader<Integer, Integer> g, long maxElements, int expirySeconds) {
     ...
     * is off.
     */
    public int statLoadsInFlight(@Nullable() Cache c, @Nullable() InternalCacheInfo inf) {
        return 0;
    }
    ...
    public interface LoaderStarter {
        ...
        void startLoad(@Nullable() CacheOperationCompletionListener l);
    }
    ...
    static class ExecutorWrapper implements Executor {
        ...
        Executor getFallBack() {
            if (fallBackExecutor == null) {
                fallBackExecutor = new ThreadPoolExecutor(0, MINIMAL_LOADER_THREADS, 21, TimeUnit.SECONDS, new SynchronousQueue<Runnable>(), HeapCache.TUNABLE.threadFactoryProvider.newThreadFactory(Thread.currentThread().getName() + "-loader-pool"), new RejectedExecutionHandler() {
        ...
        public String toString() {
            return "ExecutorWrapper{" + "fallBackExecutor=" + fallBackExecutor + ", shared=" + SHARED_EXECUTOR + '}';
        }
    }
