...
public class StaticUtil {
    ...
    public static <K, V> Cache2kBuilder<K, V> enforceWiredCache(Cache2kBuilder<K, V> b) {
        return b.addListener(new CacheEntryRemovedListener<K, V>() {
            ...
            @Override
            public void onEntryRemoved(@Nullable() Cache<K, V> cache, @Nullable() CacheEntry<K, V> entry) {
            }
        });
    }

    public static void allCachesClosed() {
        assertFalse("all caches closed", CacheManager.getInstance().getActiveCaches().iterator().hasNext());
    }
    ...
    public interface LoaderRunner {
        ...
        void run(@Nullable() CacheOperationCompletionListener l);
    }
}
