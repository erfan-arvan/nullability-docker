...
public class ChangeCapacityStressTest extends TestingBase {
    ...
    public void test() {
        ...
        final Cache<Integer, Integer> c = builder().entryCapacity(maximumSize).build();
        ...
        Runnable changeCapacity = new Runnable() {
            ...
            public void run() {
                ...
                long newCapacity = currentCapacity + maximumSize / 5 - random.nextInt((int) maximumSize / 2);
        ...
        tst.run();
        long effectiveCapacity = getInfo().getHeapCapacity();
        assertThat("cache meeting cap limit (flaky)", (long) getInternalCache().getTotalEntryCount(), Matchers.lessThanOrEqualTo(effectiveCapacity + threads));
