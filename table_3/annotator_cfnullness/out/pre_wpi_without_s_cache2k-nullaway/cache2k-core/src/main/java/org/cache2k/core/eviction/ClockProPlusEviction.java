...
public class ClockProPlusEviction extends AbstractEviction {
    ...
    public ClockProPlusEviction(HeapCacheForEviction heapCache, HeapCacheListener listener, long maxSize, Weigher weigher, long maxWeight, boolean noChunking) {
    ...
    public void checkIntegrity(IntegrityState integrityState) {
        integrityState.checkEquals("ghostSize == countGhostsInHash()", ghostSize, countGhostsInHash()).check("checkCyclicListIntegrity(handHot)", Entry.checkCyclicListIntegrity(handHot)).check("checkCyclicListIntegrity(handCold)", Entry.checkCyclicListIntegrity(handCold)).checkEquals("getCyclicListEntryCount(handHot) == hotSize", Entry.getCyclicListEntryCount(handHot), hotSize).checkEquals("getCyclicListEntryCount(handCold) == coldSize", Entry.getCyclicListEntryCount(handCold), coldSize).checkEquals("Ghost.listSize(ghostHead) == ghostSize", Ghost.listSize(ghostHead), ghostSize);
    }

    @Override
    @Nullable()
    public String getExtraStatistics() {
        return // identical to the evictions anyways
        super.getExtraStatistics() + ", coldSize=" + coldSize + ", hotSize=" + hotSize + ", hotMaxSize=" + getHotMax() + ", ghostSize=" + ghostSize + ", ghostMaxSize=" + getGhostMax() + ", coldHits=" + (coldHits + sumUpListHits(handCold)) + ", hotHits=" + (hotHits + sumUpListHits(handHot)) + ", ghostHits=" + ghostHits + ", coldRunCnt=" + coldRunCnt + ", coldScanCnt=" + coldScanCnt + ", hotRunCnt=" + hotRunCnt + ", hotScanCnt=" + hotScanCnt;
    }
