...
public abstract class EntryAction<K, V, R> extends Entry.PiggyBack implements Runnable, AsyncCacheLoader.Context<K, V>, AsyncCacheLoader.Callback<V>, AsyncCacheWriter.Callback, Progress<K, V, R> {
    ...
    static final CompletedCallback NOOP_CALLBACK = new CompletedCallback() {
        ...
        @Override
        public void entryActionCompleted(@Nullable() EntryAction ea) {
        }
    };
    ...
    public EntryAction(HeapCache<K, V> heapCache, InternalCache<K, V> userCache, Semantic<K, V, R> op, K k, Entry<K, V> e, CompletedCallback<K, V, R> cb) {
        super(null);
    ...
    public boolean isDataFreshOrRefreshing() {
        doNotCountAccess = true;
        long nrt = heapEntry.getNextRefreshTime();
        return nrt == Entry.EXPIRED_REFRESHED || nrt == Entry.EXPIRED_REFRESH_PENDING || hasFreshData();
    }
    ...
    private void onLoadFailureIntern(Throwable t) {
        newValueOrException = (V) new ExceptionWrapper<K>(key, t, loadStartedTime, heapEntry, getExceptionPropagator());
        loadCompleted();
    }
    ...
    public void mutationCalculateExpiry() {
        ...
        if (newValueOrException instanceof ExceptionWrapper) {
            ...
            try {
                ...
                if (newValueOrException == null && heapCache.isRejectNullValues() && expiry != ExpiryTimeValues.NOW) {
    ...
    private void decideForLoaderExceptionAfterExpiryCalculation(RuntimeException ouch) {
        newValueOrException = (V) new ExceptionWrapper<K>(key, ouch, loadStartedTime, heapEntry, getExceptionPropagator());
        expiry = 0;
        mutationCalculateExpiry();
    }
        ...
        if (expiredImmediately) {
            ...
            if (heapDataValid) {
                if (entryUpdatedListeners() != null) {
                    CacheEntry<K, V> previousEntry = heapCache.returnCacheEntry(heapEntry.getKey(), oldValueOrException);
    ...
    public void mutationReleaseLockAndStartTimer() {
        if (valueLoadedOrRevived) {
            if (!remove || !(heapEntry.getValueOrException() == null && heapCache.isRejectNullValues())) {
    ...
    public interface CompletedCallback<K, V, R> {
        ...
        void entryActionCompleted(@Nullable() EntryAction<K, V, R> ea);
    }
}
