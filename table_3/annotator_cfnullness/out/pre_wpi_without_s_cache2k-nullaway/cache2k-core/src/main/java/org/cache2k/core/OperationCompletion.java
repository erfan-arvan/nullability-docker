...
public class OperationCompletion<K> {
    ...
    public void complete(@Nullable() K key, Throwable t) {
        if (t != null) {
    ...
    private void allCompleted() {
        if (exceptionCount == 0) {
            ...
            listener.onException(wrap(null, exception));
        } else {
            listener.onException(wrap("finished with " + exceptionCount + " exceptions " + "out of " + initialCount + " operations" + ", one propagated as cause", exception));
        }
    }
