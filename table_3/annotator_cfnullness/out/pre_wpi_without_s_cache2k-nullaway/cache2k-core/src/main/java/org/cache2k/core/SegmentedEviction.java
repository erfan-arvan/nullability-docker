...
public class SegmentedEviction implements Eviction, EvictionMetrics {
    ...
    public void changeCapacity(long entryCountOrWeight) {
        long limitPerSegment = isWeigherPresent() ? EvictionFactory.determineMaxWeight(entryCountOrWeight, segments.length) : EvictionFactory.determineMaxSize(entryCountOrWeight, segments.length);
        for (Eviction ev : segments) {
