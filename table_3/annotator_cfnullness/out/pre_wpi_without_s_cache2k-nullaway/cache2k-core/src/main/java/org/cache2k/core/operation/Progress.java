...
public interface Progress<K, V, R> {
     ...
     */
    void entryResult(@Nullable() ExaminationEntry<K, V> e);
     ...
     */
    void expire(@Nullable() long expiryTime);
     ...
     */
    void failure(@Nullable() RuntimeException t);

    /**
     * Set new value, skip expiry calculation and set expiry time directly.
     */
    void putAndSetExpiry(@Nullable() V value, @Nullable() long expiryTime, @Nullable() long refreshTime);
}
