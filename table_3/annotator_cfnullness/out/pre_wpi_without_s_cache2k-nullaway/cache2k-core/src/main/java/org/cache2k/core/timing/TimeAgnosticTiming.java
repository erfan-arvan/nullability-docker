...
public abstract class TimeAgnosticTiming<K, V> extends Timing<K, V> {
    ...
    static class EternalImmediate<K, V> extends TimeAgnosticTiming<K, V> {
        ...
        @Override
        public long calculateNextRefreshTime(@Nullable() Entry<K, V> e, @Nullable() V v, @Nullable() long loadTime) {
            return ExpiryPolicy.ETERNAL;
        }

        @Override
        public long cacheExceptionUntil(@Nullable() Entry<K, V> e, @Nullable() LoadExceptionInfo inf) {
            return 0;
        }

        @Override
        public long suppressExceptionUntil(@Nullable() Entry<K, V> e, @Nullable() LoadExceptionInfo inf) {
            return 0;
        }
    }
