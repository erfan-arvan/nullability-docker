...
public class StaticTiming<K, V> extends Timing<K, V> {
    ...
    StaticTiming(InternalCacheBuildContext<K, V> buildContext, ResiliencePolicy<K, V> resiliencePolicy) {
        ...
        if (c.getExpireAfterWrite() == null || c.getExpireAfterWrite() == Cache2kConfig.EXPIRY_ETERNAL) {
    ...
    static long limitExpiryToMaxLinger(long now, long maxLinger, long requestedExpiryTime, boolean sharpExpiryEnabled) {
        if (sharpExpiryEnabled && requestedExpiryTime > ExpiryPolicy.REFRESH && requestedExpiryTime < ExpiryPolicy.ETERNAL) {
