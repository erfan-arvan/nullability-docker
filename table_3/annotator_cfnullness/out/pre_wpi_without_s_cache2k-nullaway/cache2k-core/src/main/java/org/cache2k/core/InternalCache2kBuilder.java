...
public class InternalCache2kBuilder<K, V> implements InternalCacheBuildContext<K, V> {
        ...
        if (config.getLoader() != null) {
            ...
            if (obj instanceof CacheLoader) {
                ...
                c.setAdvancedLoader(new AdvancedCacheLoader<K, V>() {
                    ...
                    @Override
                    public V load(K key, @Nullable() long startTime, @Nullable() CacheEntry<K, V> currentEntry) throws Exception {
     ...
     */
    private static class WrappedAdvancedCacheLoader<K, V> implements AdvancedCacheLoader<K, V>, Closeable {
    ...
    public Cache<K, V> buildAsIs() {
        ...
        CoreConfig coreConfig = config.getSections().getSection(CoreConfig.class, CoreConfig.DEFAULT);
        ...
        bc.setClock(clock);
        if (config.isRefreshAhead() && !(config.getAsyncLoader() != null || config.getLoader() != null || config.getAdvancedLoader() != null)) {
            throw new IllegalArgumentException("refresh ahead enabled, but no loader defined");
        }
        boolean wrap = config.getWeigher() != null || config.hasListeners() || config.hasAsyncListeners() || config.getWriter() != null || config.getAsyncLoader() != null;
        ...
        if (wrap) {
            ...
            if (config.hasListeners()) {
                for (CustomizationSupplier<CacheEntryOperationListener<K, V>> f : config.getListeners()) {
                    CacheEntryOperationListener<K, V> el = createCustomization(f);
            ...
            bc.eviction = EVICTION_FACTORY.constructEviction(this, bc, wc, config, Runtime.getRuntime().availableProcessors());
            ...
            Timing rh = Timing.of(this);
            bc.setTiming(rh);
            bc.eviction = EVICTION_FACTORY.constructEviction(this, bc, HeapCacheListener.NO_OPERATION, config, Runtime.getRuntime().availableProcessors());
            bc.init();
        }
        if (config.hasLifecycleListeners()) {
            List<CacheClosedListener> closedListeners = new ArrayList<CacheClosedListener>();
            for (CustomizationSupplier<? extends CacheLifecycleListener> sup : config.getLifecycleListeners()) {
    ...
    private void checkConfiguration() {
        if (config.getExpireAfterWrite() == Cache2kConfig.EXPIRY_NOT_ETERNAL && config.getExpiryPolicy() == null) {
    ...
    private static class AsyncUpdatedListener<K, V> implements CacheEntryUpdatedListener<K, V> {
        ...
        @Override
        public void onEntryUpdated(Cache<K, V> cache, CacheEntry<K, V> currentEntry, CacheEntry<K, V> entryWithNewData) {
            dispatcher.queue(new AsyncEvent<K>() {
