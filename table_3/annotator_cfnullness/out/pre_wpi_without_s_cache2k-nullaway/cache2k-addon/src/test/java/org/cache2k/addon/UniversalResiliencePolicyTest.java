...
public class UniversalResiliencePolicyTest {
    ...
    private Cache2kBuilder<Integer, Integer> builder() {
        return new Cache2kBuilder<Integer, Integer>() {
            ...
            CustomizationSupplier<? extends ResiliencePolicy> configuredSupplier = ctx.getConfig().getResiliencePolicy();
    ...
    public void configVariant_setSupplier() {
        cache = builder().set(cfg -> cfg.setResiliencePolicy(UniversalResiliencePolicy.supplier())).with(UniversalResilienceConfig.class, builder -> builder.resilienceDuration(0, TimeUnit.MILLISECONDS)).build();
    }

    /* An exhausting set of alternatives to enable the policy and disable it. */
    @Test
    public void configVariant_enableDisable() {
        cache = builder().set(cfg -> cfg.setResiliencePolicy(UniversalResiliencePolicy.supplier())).setup(b -> b.config().setResiliencePolicy(UniversalResiliencePolicy.supplier())).setup(UniversalResiliencePolicy::enable).setup(ResiliencePolicy::disable).build();
    }

    /* enable and add config section in single command */
    @Test
    public void configVariant_singleApply() {
        cache = builder().setupWith(UniversalResiliencePolicy::enable, b -> b.resilienceDuration(4711, TimeUnit.MILLISECONDS)).build();
    }
    ...
    public void configVariant_overwrites() {
        cache = builder().setupWith(UniversalResiliencePolicy::enable, b -> b.resilienceDuration(4711, TimeUnit.MILLISECONDS).retryInterval(1234, TimeUnit.MILLISECONDS))./* Overwrite the default later ...  */
        with(UniversalResilienceConfig.class, builder -> builder.resilienceDuration(123, TimeUnit.MILLISECONDS))./* Results in combined section parameters */
    ...
    public void expiry0_any() {
        ...
        }.setup(UniversalResiliencePolicy::enable).expireAfterWrite(0, TimeUnit.MINUTES)./* ... set loader ... */
        build();
        assertNull(policy);
        cache.put(1, 123);
    ...
    public void enableDisable() {
        cache = builder().setup(UniversalResiliencePolicy::enable).setup(ResiliencePolicy::disable).build();
        assertNull(policy);
    }

    @Test
    public void enableDisableViaResilience0() {
        cache = builder().setup(UniversalResiliencePolicy::enable).with(UniversalResilienceConfig.class, builder -> builder.resilienceDuration(0, TimeUnit.MILLISECONDS)).build();
        assertNull(policy);
    }
    ...
    public void noExpiry_noResilienceParameters() {
        cache = builder().set(cfg -> cfg.setResiliencePolicy(UniversalResiliencePolicy.supplier()))./* ... set loader ... */
        build();
        assertNull(policy);
    }
    ...
    public void expiryPolicy() {
        cache = builder().setup(UniversalResiliencePolicy::enable).expiryPolicy((key, value, loadTime, oldEntry) -> 0).with(UniversalResilienceConfig.class, b -> b.resilienceDuration(30, TimeUnit.SECONDS))./* ... set loader ... */
        build();
    ...
    public void expiry10m() {
        cache = builder().setup(UniversalResiliencePolicy::enable).expireAfterWrite(10, TimeUnit.MINUTES)./* ... set loader ... */
        build();
    ...
    public void expiry10m_duration30s() {
        cache = builder().setupWith(UniversalResiliencePolicy::enable, b -> b.resilienceDuration(30, TimeUnit.SECONDS)).expireAfterWrite(10, TimeUnit.MINUTES).loader(this::load).build();
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getResilienceDuration());
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getMaxRetryInterval());
        assertEquals(TimeUnit.SECONDS.toMillis(3), policy.getRetryInterval());
        long t0 = System.currentTimeMillis();
        cache.get(1);
        Assertions.assertThat((long) cache.invoke(1, e -> e.getExpiryTime())).isGreaterThanOrEqualTo(t0 + TimeUnit.MINUTES.toMillis(10));
        ...
        Assertions.assertThat((long) cache.invoke(3, e -> {
            e.getException();
            return e.getExpiryTime();
        })).as("Retry").isGreaterThanOrEqualTo(t0 + TimeUnit.SECONDS.toMillis(3));
        Assertions.assertThat((long) cache.invoke(2, e -> {
            e.load();
            assertEquals(2, (int) e.getValue());
            return e.getExpiryTime();
        })).as("Suppress is doing retry as well").isGreaterThanOrEqualTo(t0 + TimeUnit.SECONDS.toMillis(3));
    }

    @Test
    public void expiry10m_retry10s() {
        cache = builder().expireAfterWrite(10, TimeUnit.MINUTES).setupWith(UniversalResiliencePolicy::enable, b -> b.retryInterval(10, TimeUnit.SECONDS))./* ... set loader ... */
        build();
        assertEquals(TimeUnit.MINUTES.toMillis(10), policy.getResilienceDuration());
    ...
    public void eternal_duration30s() {
        cache = builder().eternal(true).setup(UniversalResiliencePolicy::enable).with(UniversalResilienceConfig.class, b -> b.resilienceDuration(30, TimeUnit.SECONDS))./* ... set loader ... */
        build();
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getResilienceDuration());
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getMaxRetryInterval());
        assertEquals(TimeUnit.SECONDS.toMillis(3), policy.getRetryInterval());
    ...
    public void eternal_duration30s_retry10s() {
        cache = builder().eternal(true).setupWith(UniversalResiliencePolicy::enable, b -> b.resilienceDuration(30, TimeUnit.SECONDS).retryInterval(10, TimeUnit.SECONDS)).loader(this::load).build();
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getResilienceDuration());
        assertEquals(TimeUnit.SECONDS.toMillis(30), policy.getMaxRetryInterval());
        assertEquals(TimeUnit.SECONDS.toMillis(10), policy.getRetryInterval());
    }
    ...
    public void eternal_retry10s() {
        cache = builder().eternal(true).setupWith(UniversalResiliencePolicy::enable, b -> b.retryInterval(10, TimeUnit.SECONDS))./* ... set loader ... */
        build();
