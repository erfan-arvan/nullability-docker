...
public class UniversalResilienceSupplier<K, V> implements CustomizationSupplier<ResiliencePolicy<K, V>>, WithSection<UniversalResilienceConfig, UniversalResilienceConfig.Builder> {
    ...
    static <K, V> ResiliencePolicy<K, V> supplyPolicy(Cache2kConfig<K, V> rootCfg) {
        UniversalResilienceConfig cfg = rootCfg.getSections().getSection(UniversalResilienceConfig.class, DEFAULT_CONFIG);
        ...
        UniversalResiliencePolicy<K, V> policy = new UniversalResiliencePolicy<>(resilienceDuration, maxRetryInterval, retryInterval, cfg.getBackoffMultiplier());
        return policy;
    }
