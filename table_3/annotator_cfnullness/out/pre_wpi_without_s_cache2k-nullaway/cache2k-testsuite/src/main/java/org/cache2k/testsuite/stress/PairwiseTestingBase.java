...
public class PairwiseTestingBase {
    ...
    @Test
    public void test() {
        Cache<Integer, Integer> c = augmenter.augment(Cache2kBuilder.of(Integer.class, Integer.class)).build();
    ...
    Iterable<CacheKeyActorPair<?, Integer, Integer>> actorPairs(Class<?> c) {
        List<CacheKeyActorPair<?, Integer, Integer>> l = new ArrayList<CacheKeyActorPair<?, Integer, Integer>>();
    ...
    interface BuilderAugmenter {
        ...
        <K, V> Cache2kBuilder<K, V> augment(@Nullable() Cache2kBuilder<K, V> b);
    }
}
