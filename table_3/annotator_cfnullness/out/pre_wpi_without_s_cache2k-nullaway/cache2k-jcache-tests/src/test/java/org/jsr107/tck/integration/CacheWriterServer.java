...
public class CacheWriterServer<K, V> extends Server {
    ...
    public class WriteAllOperationHandler implements OperationHandler {
        ...
        private Collection<Cache.Entry<? extends K, ? extends V>> readEntries(ObjectInputStream ois) throws IOException, ClassNotFoundException {
            Collection<Cache.Entry<? extends K, ? extends V>> entrys = new HashSet<Cache.Entry<? extends K, ? extends V>>();
            K key = (K) ois.readObject();
            V value = null;
            if (key != null) {
                value = (V) ois.readObject();
            }
            Entry entry = ((key == null) || (value == null)) ? null : new Entry(key, value);
            while (entry != null) {
                entrys.add(entry);
                key = (K) ois.readObject();
                value = null;
                ...
                entry = ((key == null) || (value == null)) ? null : new Entry(key, value);
            }
            return entrys;
        }
