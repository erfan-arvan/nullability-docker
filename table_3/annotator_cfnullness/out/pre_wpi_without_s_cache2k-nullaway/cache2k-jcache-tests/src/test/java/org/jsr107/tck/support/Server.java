...
public class Server implements AutoCloseable {
     ...
     */
    private static final boolean ALLOW_DIRECT_CALLS = System.getProperty("org.jsr107.tck.support.server.alwaysUseTcp") == null;
    ...
    public static final Operation<Void> CLOSE_OPERATION = new Operation<Void>() {
         ...
         */
        @Nullable()
        public Void onInvoke(final ObjectInputStream ois, @Nullable() final ObjectOutputStream oos) throws IOException {
            try {
                ois.readByte();
    ...
    public synchronized void closeWasCalledOnClient() {
        if (serverDirectUsage == 0) {
            throw new IllegalStateException("customization already closed, class: " + this.getClass().getSimpleName());
        }
        serverDirectUsage--;
    }
    ...
    public synchronized void close() {
        if (serverThread != null) {
            ...
            if (ALLOW_DIRECT_CALLS) {
                if (serverDirectUsage > 0) {
                    throw new IllegalStateException("The close() method was not called. " + "Customizations implementing Closeable need to be closed. " + "See https://github.com/jsr107/jsr107tck/issues/100" + ", server type " + this.getClass().getSimpleName());
                }
            }
            if (!serverThread.wasUsed()) {
                ...
                if (clientConnections.size() > 0) {
                    ...
                    throw new IllegalStateException("Excepting no open client connections. " + "Customizations implementing Closeable need to be closed. " + "See https://github.com/jsr107/jsr107tck/issues/100" + ", server type " + this.getClass().getSimpleName());
                }
    ...
    private ServerSocket createServerSocket() throws IOException {
        ...
        LOG.log(Level.INFO, "Starting " + this.getClass().getCanonicalName() + " server at address:" + getServerInetAddress() + " port:" + result.getLocalPort());
        return result;
    }
            ...
            if (serverSocketAddress == null) {
                ...
                try {
                    ...
                    if (serverSocketNetworkInterface != null) {
                        serverSocketAddress = getFirstNonLoopbackAddress(serverSocketNetworkInterface, preferIPV4Stack, preferIPV6Addresses);
                    }
                    if (serverSocketAddress == null) {
