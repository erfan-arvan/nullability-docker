 ...
 *
 * <p>Change from original TCK implementation: Only initialize listener or expiry policy
 * if needed.</p>
 *
 * @author Brian Oliver
 * @author Joe Fialli
...
public class CacheExpiryTest extends CacheTestSupport<Integer, Integer> {
    ...
    private void setupExpiryPolicyClientServer(ExpiryPolicy policy) {
        assert expiryPolicyServer == null;
        ...
        expiryPolicyClient = new ExpiryPolicyClient(expiryPolicyServer.getInetAddress(), expiryPolicyServer.getPort());
    }

    public void setupListenerClientServer() {
        try {
            ...
            cacheEntryListerClient = new CacheEntryListenerClient<>(cacheEntryListenerServer.getInetAddress(), cacheEntryListenerServer.getPort());
        } catch (IOException ex) {
            throw new RuntimeException(ex);
        }
    }
    ...
    protected MutableConfiguration<Integer, Integer> extraSetup(MutableConfiguration<Integer, Integer> configuration) {
        if (cacheEntryListenerServer == null) {
        ...
        listenerConfiguration = new MutableCacheEntryListenerConfiguration<>(FactoryBuilder.factoryOf(cacheEntryListerClient), null, true, true);
