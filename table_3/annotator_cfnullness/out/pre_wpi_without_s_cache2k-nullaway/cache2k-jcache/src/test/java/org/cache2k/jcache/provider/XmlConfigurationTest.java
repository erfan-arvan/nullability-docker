...
public class XmlConfigurationTest {
    ...
    public void sectionIsThere() {
        ...
        }.manager(CacheManager.getInstance(MANAGER_NAME)).name("withSection");
    ...
    public void sectionIsThereViaStandardElementName() {
        Cache2kBuilder<String, String> b = new Cache2kBuilder<String, String>() {
        }.manager(CacheManager.getInstance(MANAGER_NAME)).name("withJCacheSection");
        Cache2kConfig<String, String> cfg = b.config();
        Cache<String, String> c = b.build();
        assertEquals("default is false", false, new JCacheConfig().isCopyAlwaysIfRequested());
        assertNotNull("section present", cfg.getSections().getSection(JCacheConfig.class));
    ...
    public void configurationMissing() throws Exception {
        javax.cache.CacheManager _manager = Caching.getCachingProvider().getCacheManager(new URI(MANAGER_NAME), null);
    ...
    public void standardJCacheSemanticsIfNoExternalConfiguration() throws Exception {
        CachingProvider p = Caching.getCachingProvider();
        javax.cache.CacheManager cm = p.getCacheManager();
        javax.cache.Cache<String, BigDecimal> c = cm.createCache("test", new MutableConfiguration<String, BigDecimal>());
    ...
    public void cache2kSemanticsIfEmptyConfigurationPresent() throws Exception {
        ...
        javax.cache.Cache<String, BigDecimal> c = cm.createCache("test2", new MutableConfiguration<String, BigDecimal>());
    ...
    public void jcacheWithLoaderButNoReadThroughEnabled() throws Exception {
        javax.cache.CacheManager _manager = Caching.getCachingProvider().getCacheManager(new URI(MANAGER_NAME), null);
        javax.cache.Cache<Integer, String> _cache = _manager.getCache("withCache2kLoaderNoReadThrough");
    ...
    public void jcacheWithLoaderReadThroughEnabled() throws Exception {
        javax.cache.CacheManager _manager = Caching.getCachingProvider().getCacheManager(new URI(MANAGER_NAME), null);
        javax.cache.Cache<Integer, String> _cache = _manager.getCache("withCache2kLoaderWithReadThrough");
        assertNotNull(_cache.get(123));
    }
    ...
    public void readAllXml() {
        ...
        }.manager(CacheManager.getInstance("all")).name("hello").build();
        c.close();
    }

    @Test
    public void validateCoreXsd() throws Exception {
        Source cfg = new StreamSource(getClass().getResourceAsStream("/cache2k-all.xml"));
        SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        Schema schema = schemaFactory.newSchema(Constants.class.getResource(Constants.CORE_SCHEMA_LOCATION));
    ...
    public void validateVariableExpansion() {
        ...
        }.manager(CacheManager.getInstance("all")).name("jcache1");
    ...
    public void checkSectionSetter() {
        ...
        assertThat(cfg.getSections().toString(), CoreMatchers.containsString("SectionContainer"));
