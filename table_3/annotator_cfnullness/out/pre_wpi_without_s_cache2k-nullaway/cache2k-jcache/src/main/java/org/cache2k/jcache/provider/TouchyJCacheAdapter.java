...
public class TouchyJCacheAdapter<K, V> implements Cache<K, V> {
    ...
    public boolean replace(K key, final V oldValue, final V newValue) {
        ...
        checkNullValue(oldValue);
        return c2kCache.invoke(key, new EntryProcessor<K, V, Boolean>() {
    ...
    private <T> javax.cache.processor.EntryProcessor<K, V, T> wrapEntryProcessor(final javax.cache.processor.EntryProcessor<K, V, T> ep) {
        ...
        return new javax.cache.processor.EntryProcessor<K, V, T>() {
            ...
            @Override
            public T process(final MutableEntry<K, V> e0, Object... args) throws EntryProcessorException {
                MutableEntry<K, V> me = new MutableEntry<K, V>() {
                    ...
                    @Override
                    public <X> X unwrap(@Nullable() Class<X> clazz) {
                        return null;
                    }
                };
    ...
    public static class ExpiryPolicyAdapter<K, V> implements org.cache2k.expiry.ExpiryPolicy<K, V>, Closeable {
        ...
        public long calculateExpiryTime(@Nullable() K key, V value, long loadTime, CacheEntry<K, V> currentEntry) {
            ...
            return loadTime + d.getTimeUnit().toMillis(d.getDurationAmount());
        }
