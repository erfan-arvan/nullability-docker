...
public class JCacheBuilder<K, V> {
    ...
    public void setConfiguration(Configuration<K, V> cfg) {
        if (cfg instanceof CompleteConfiguration) {
            ...
            if (cfg instanceof ExtendedConfiguration) {
                ...
                if (cache2kConfig != null) {
                    if (cache2kConfig.getName() != null && !cache2kConfig.getName().equals(name)) {
        ...
        if (cache2kConfig == null) {
            cache2kConfig = CacheManagerImpl.PROVIDER.getDefaultConfig(manager.getCache2kManager());
        ...
        Cache2kCoreProviderImpl.CACHE_CONFIGURATION_PROVIDER.augmentConfig(manager.getCache2kManager(), cache2kConfig);
        ...
        if (cache2kConfigurationWasProvided) {
            ...
            JCacheConfig extraConfigurationSpecified = cache2kConfig.getSections().getSection(JCacheConfig.class);
    ...
    public Cache<K, V> build() {
        ...
        setupExpiryPolicy();
        setupEventHandling();
        cache2kConfig.getLifecycleListeners().add(new CustomizationReferenceSupplier<CacheClosedListener>(JCacheJmxSupport.SINGLETON));
        buildAdapterCache();
        wrapForExpiryPolicy();
        wrapIfCopyIsNeeded();
        return createdCache;
    ...
    private void setupExceptionPropagator() {
        ...
        cache2kConfig.setExceptionPropagator(new CustomizationReferenceSupplier<ExceptionPropagator<K>>(new ExceptionPropagator<K>() {

            @Override
            public RuntimeException propagateException(LoadExceptionInfo loadExceptionInfo) {
                return new CacheLoaderException("propagate previous loader exception", loadExceptionInfo.getException());
            }
        }));
    }
    ...
    private void setupCacheThrough() {
        if (config.getCacheLoaderFactory() != null) {
            ...
            cache2kConfig.setAdvancedLoader(new CustomizationReferenceSupplier<AdvancedCacheLoader<K, V>>(new CloseableLoader() {
                ...
                @Override
                public V load(K key, @Nullable() long startTime, @Nullable() CacheEntry<K, V> currentEntry) {
                    return clf.load(key);
                }
            }));
        }
        if (config.getCacheWriterFactory() != null) {
            ...
            cache2kConfig.setWriter(new CustomizationReferenceSupplier<CacheWriter<K, V>>(new CloseableWriter() {
                ...
                public void write(final K key, final V value) {
                    Cache.Entry<K, V> ce = new Cache.Entry<K, V>() {
                        ...
                        @Override
                        public <T> T unwrap(@Nullable() Class<T> clazz) {
    ...
    private void setupExpiryPolicy() {
        ...
        if (expiryPolicy == null || expiryPolicy instanceof EternalExpiryPolicy) {
            cache2kConfig.setExpiryPolicy(new CustomizationReferenceSupplier<org.cache2k.expiry.ExpiryPolicy<K, V>>(new org.cache2k.expiry.ExpiryPolicy<K, V>() {

                @Override
                public long calculateExpiryTime(@Nullable() K key, V value, @Nullable() long loadTime, @Nullable() CacheEntry<K, V> currentEntry) {
                    if (value == null) {
                        return NOW;
                    }
                    return ETERNAL;
        ...
        if (expiryPolicy instanceof ModifiedExpiryPolicy) {
            ...
            cache2kConfig.setExpiryPolicy(new CustomizationReferenceSupplier<org.cache2k.expiry.ExpiryPolicy<K, V>>(new org.cache2k.expiry.ExpiryPolicy<K, V>() {
                ...
                @Override
                public long calculateExpiryTime(@Nullable() K key, V value, long loadTime, @Nullable() CacheEntry<K, V> currentEntry) {
                    if (value == null) {
                        return NOW;
                    }
        ...
        if (expiryPolicy instanceof CreatedExpiryPolicy) {
            ...
            cache2kConfig.setExpiryPolicy(new CustomizationReferenceSupplier<org.cache2k.expiry.ExpiryPolicy<K, V>>(new org.cache2k.expiry.ExpiryPolicy<K, V>() {
                ...
                @Override
                public long calculateExpiryTime(@Nullable() K key, V value, long loadTime, CacheEntry<K, V> currentEntry) {
                    if (value == null) {
                        return NOW;
                    }
        ...
        cache2kConfig.setExpiryPolicy(new CustomizationReferenceSupplier<org.cache2k.expiry.ExpiryPolicy<K, V>>(new TouchyJCacheAdapter.ExpiryPolicyAdapter<K, V>(expiryPolicy)));
    }

    @SuppressWarnings("unchecked")
    private void setupEventHandling() {
        if ((config.getCacheEntryListenerConfigurations() == null || !config.getCacheEntryListenerConfigurations().iterator().hasNext()) && !extraConfiguration.isSupportOnlineListenerAttachment()) {
            eventHandling = (EventHandling<K, V>) EventHandling.DISABLED;
            return;
        }
        EventHandlingImpl<K, V> eventHandling = new EventHandlingImpl<K, V>(manager, Executors.newCachedThreadPool());
        ...
        cache2kConfig.getLifecycleListeners().add(new CustomizationReferenceSupplier<CacheClosedListener>(eventHandling));
    }

    private void buildAdapterCache() {
        createdCache = new JCacheAdapter<K, V>(manager, new InternalCache2kBuilder<K, V>(cache2kConfig, manager.getCache2kManager()).buildAsIs(), keyType.getType(), valueType.getType(), config.isStoreByValue(), config.isReadThrough() || extraConfiguration.isEnableReadThrough(), config.getCacheLoaderFactory() != null || cache2kConfig.getLoader() != null || cache2kConfig.getAdvancedLoader() != null || cache2kConfig.getAsyncLoader() != null, eventHandling);
    }
    ...
    private void wrapIfCopyIsNeeded() {
        if (extraConfiguration.isCopyAlwaysIfRequested() && config.isStoreByValue()) {
            ...
            createdCache = new CopyCacheProxy<K, V>(createdCache, keyTransformer, valueTransformer);
        }
    }
    ...
    private <T> ObjectTransformer<T, T> createCopyTransformer(CacheType<T> type) {
        ...
        ObjectTransformer<T, T> keyTransformer = f.createCopyTransformer(type.getType(), manager.getClassLoader());
        if (keyTransformer == null) {
            keyTransformer = (ObjectTransformer<T, T>) new RuntimeCopyTransformer(manager.getClassLoader());
        }
