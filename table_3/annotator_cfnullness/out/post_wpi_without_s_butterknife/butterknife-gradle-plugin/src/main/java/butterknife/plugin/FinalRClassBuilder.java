...
public final class FinalRClassBuilder {
    ...
    private static final String[] SUPPORTED_TYPES = { "anim", "array", "attr", "bool", "color", "dimen", "drawable", "id", "integer", "layout", "menu", "plurals", "string", "style", "styleable" };

    private FinalRClassBuilder() {
    }

    public static void brewJava(File rFile, File outputDir, String packageName, String className, boolean useAndroidX) throws Exception {
    ...
    private static void addResourceType(List<String> supportedTypes, TypeSpec.Builder result, ClassOrInterfaceDeclaration node, boolean useAndroidX) {
        ...
        for (BodyDeclaration field : node.getMembers()) {
            if (field instanceof FieldDeclaration) {
                ...
                if (isInt(declaration)) {
                    addResourceField(resourceType, declaration.getVariables().get(0), getSupportAnnotationClass(type, useAndroidX));
                }
            }
        }
    ...
    private static void addResourceField(TypeSpec.Builder resourceType, VariableDeclarator variable, ClassName annotation) {
        ...
        String fieldValue = variable.getInitializer().map(Node::toString).orElseThrow(() -> new IllegalStateException("Field " + fieldName + " missing initializer"));
        FieldSpec.Builder fieldSpecBuilder = FieldSpec.builder(int.class, fieldName).addModifiers(PUBLIC, STATIC, FINAL).initializer(fieldValue);
        if (annotation != null) {
