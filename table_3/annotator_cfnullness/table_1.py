...
declare -A lib_copies=(
  ["picasso_pre_wpi"]="pre_wpi_without_s_picasso"
  ["picasso_post_wpi"]="post_wpi_without_s_picasso"
  ["butterknife_pre_wpi"]="pre_wpi_without_s_butterknife"
  ["butterknife_post_wpi"]="post_wpi_without_s_butterknife"
)
...
for result_file in "$WPI_LOG_DIR"/*.txt; do
  filename=$(basename "$result_file")  # e.g., cache2k-nullaway_pre_wpi-before-wpi.txt
  benchmark="${filename%-before-wpi.txt}"  # strip suffix, now we have: project_version

  # More robust split: splits at the first underscore, handles dashes in project names
  project="${benchmark%%_*}"            # everything before the FIRST underscore
  version="${benchmark#*_}"             # everything after the FIRST underscore

  # Optional: debug check
  echo "📂 Parsing result: $filename → project=$project, version=$version"

  cf_type="CFnullness"
  [[ "$project" == *"nullaway"* ]] && cf_type="nullaway"

  # Ensure the old copy (if any) is removed to prevent duplicates
  rm -f "$RESULTS_DIR/${project}-${version}-${cf_type}.txt"

  cp "$result_file" "$RESULTS_DIR/${project}-${version}-${cf_type}.txt"
done
  ...
  for version in pre_wpi post_wpi; do
    ...
    if [[ "$project" == cache2k* ]]; then
      ...
      if [ -d "$sub" ]; then
        ...
        cf_type="nullaway"
        [[ "$project" == *"CFNullness" ]] && cf_type="CFnullness"
        mvn clean > /dev/null 2>&1
	rm -f "$RESULTS_DIR/${project}-${version}-${cf_type}.txt"
        mvn compile -PCheckerFramework > "$RESULTS_DIR/${project}-${version}-${cf_type}.txt" 2>&1

        echo "Running count script"
        echo -e "\n==== Count Script Output ====\n" >> "$RESULTS_DIR/${project}-${version}-${cf_type}.txt"
        bash "$COUNT_SCRIPT" >> "$RESULTS_DIR/${project}-${version}-${cf_type}.txt"
        cd "$BASE_DIR"
      fi
    fi
    ...
    if [[ "$project" == table-* ]]; then
      if [ -d "$proj_dir" ]; then
        ...
        echo "Running mvn compile -PCheckerFramework"
        mvn clean > /dev/null 2>&1
	rm -f "$RESULTS_DIR/${project}-${version}-CFnullness.txt"
        mvn compile -PCheckerFramework > "$RESULTS_DIR/${project}-${version}-CFnullness.txt" 2>&1

        echo "Running count script"
        echo -e "\n==== Count Script Output ====\n" >> "$RESULTS_DIR/${project}-${version}-CFnullness.txt"
        bash "$COUNT_SCRIPT" >> "$RESULTS_DIR/${project}-${version}-CFnullness.txt"
        cd "$BASE_DIR"
      fi
    fi
