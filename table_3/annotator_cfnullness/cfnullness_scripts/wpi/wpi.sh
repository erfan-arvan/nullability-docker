...
unzip_jars() {
    ...
    unzip -o -q "$jar_file" -d "$extract_to" || true
    done
}
...
# find ${WPITEMPDIR} -name "*KeyForSubchecker.ajava" -type f -exec rm {} +
# find ${WPIOUTDIR} -name "*KeyForSubchecker.ajava" -type f -exec rm {} +
